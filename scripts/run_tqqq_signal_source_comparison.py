#!/usr/bin/env python
"""Compare TQQQ-derived vs QQQ-derived MACD signals for trading synthetic TQQQ."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-prefix", default="tqqq_signal_source_comparison")
...
def _load_original_keep_cases(min_ann_return: float) -> pd.DataFrame:
    metrics_path = Path("reports/tables/tqqq_entry_signal_comparison_metrics.csv")
    if not metrics_path.exists():
        raise FileNotFoundError(
            "Run scripts/run_tqqq_entry_signal_comparison.py first so the original "
            "TQQQ-signal metrics exist."
        )
    original = pd.read_csv(metrics_path)
    original = original[
        original["name"].str.startswith("macd_")
        & original["annualized_return"].ge(min_ann_return)
    ].copy()
    if original.empty:
        raise RuntimeError("No original MACD cases passed the annualized-return filter.")
    return original.sort_values("sharpe_ratio", ascending=False)


def _parse_original_case_name(name: str) -> tuple[str, str, str, bool]:
    """Map prior table names into (average_type, unit, entry_mode, profit_lock)."""
    profit_lock = name.endswith("_profit_lock_150_250")
    base = name.removesuffix("_profit_lock_150_250").removesuffix("_full")
    parts = base.split("_")
    # Names look like macd_ema_bars_hist_rising or
    # macd_sma_days_hist_gt_0.
    ...
    return average_type, unit, entry_mode, profit_lock


def _build_raw_variant(
    ...
    *,
    ...
    raw = raw.rename(target_ticker)
    if not profit_lock:
        return raw.to_frame(target_ticker), {
            "profit_lock_150pct_hit_count": np.nan,
            "profit_lock_250pct_hit_count": np.nan,
        }
    weights = trade_profit_lock_tiers(
        raw,
        target_price,
        thresholds_to_weights=[(1.50, 0.75), (2.50, 0.50)],
    )
    return weights.to_frame(target_ticker), {
        "profit_lock_150pct_hit_count": float(
            count_profit_lock_hits(raw, target_price, threshold=1.50)
        ),
        "profit_lock_250pct_hit_count": float(
            count_profit_lock_hits(raw, target_price, threshold=2.50)
        ),
    }
...
def main() -> None:
    ...
    keep_cases = _load_original_keep_cases(args.min_original_ann_return)
    raw_variants: dict[str, pd.DataFrame] = {}
    meta_by_name: dict[str, dict[str, Any]] = {}

    signal_sources = {
        "signal_tqqq": target_prices[args.target_ticker],
        "signal_qqq": qqq_prices[args.benchmark_ticker],
    }
    for original_name in keep_cases["name"]:
        average_type, unit, entry_mode, profit_lock = _parse_original_case_name(original_name)
        for source_name, signal_price in signal_sources.items():
            variant_name = f"{source_name}__{original_name}"
            raw_weights, hit_counts = _build_raw_variant(
                signal_price,
                allowed_regime=allowed_regime,
                bars_per_day=bars_per_day,
            ...
            raw_variants[variant_name] = raw_weights
            meta_by_name[variant_name] = {
                "original_case": original_name,
                "signal_source": source_name.removeprefix("signal_"),
                "average_type": average_type,
                "macd_unit": unit,
                "entry_mode": entry_mode,
                "profit_lock": profit_lock,
    ...
    for name, raw_weights in raw_variants.items():
        ...
        metrics.update(
            {
                "name": name,
                "strategy": "signal_source_comparison",
                "segment": "full_sample",
    ...
    comparison_path = tables_dir / f"{args.output_prefix}_paired_deltas.csv"
    ...
    strategy_metrics = metrics[metrics["strategy"].eq("signal_source_comparison")]
    paired = strategy_metrics.pivot(index="original_case", columns="signal_source")
    ...
    for column in [
        ...
        deltas[f"{column}_tqqq_signal"] = paired[column]["tqqq"]
        deltas[f"{column}_qqq_signal"] = paired[column]["qqq"]
        deltas[f"{column}_qqq_minus_tqqq"] = paired[column]["qqq"] - paired[column]["tqqq"]
    deltas = deltas.reset_index()

    metrics.to_csv(metrics_path, index=False)
    compact.to_csv(compact_path, index=False)
    deltas.to_csv(comparison_path, index=False)
    ...
    _plot_top(
        ...
        metrics,
        plot_path,
        title="Synthetic TQQQ: TQQQ-signal vs QQQ-signal MACD",
    )

    print(f"Metrics saved to {metrics_path}")
    print(f"Compact table saved to {compact_path}")
    print(f"Paired deltas saved to {comparison_path}")
